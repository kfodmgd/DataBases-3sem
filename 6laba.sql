...
вы-числяющий для каждого типа аудиторий максимальную, минимальную, 
среднюю вместимость аудиторий, суммарную вме-стимость всех 
аудиторий и общее количе-ство аудиторий данного типа*/
select AUDITORIUM_TYPE.AUDITORIUM_TYPENAME,
		min(AUDITORIUM_CAPACITY)[Минимальная вместимость],
		max(AUDITORIUM_CAPACITY)[MAX],
		count(*)				[Колво],
		avg(AUDITORIUM_CAPACITY)[средняя вместимость],
		max(AUDITORIUM_CAPACITY)[Сумма]
from AUDITORIUM inner join AUDITORIUM_TYPE
	on AUDITORIUM.AUDITORIUM_TYPE=AUDITORIUM_TYPE.AUDITORIUM_TYPE
	group by AUDITORIUM_TYPENAME

/*Разработать запрос на основе таблицы PROGRESS, который содержит 
количество экзаменационных оценок в заданном интер-вале. При этом 
учесть, что сортировка строк должна осуществляться в порядке, 
обратном величине оценки; сумма значений в столбце количество 
должна быть равна количеству строк в таблице PROGRESS. */
select *
from (select case when NOTE = 10 then '10'
	...
	from PROGRESS group by case when NOTE = 10 then '10'
								when NOTE in (8,9) then '8-9'
								when NOTE in (6,7) then '6-7'
								when NOTE in (4,5) then '4-5'
								else '<4' 
...
/*Разработать SELECT-запроса на основе таблиц FACULTY, GROUPS, 
STUDENT и PROGRESS, который содержит среднюю экзаменационную оценку
для каждого кур-са каждой специальности. Строки отсорти-ровать в 
порядке убывания средней оценки*/
select FACULTY.FACULTY, GROUPS.PROFESSION, GROUPS.YEAR_FIRST, round(avg(cast(PROGRESS.NOTE as float(4))),2)[среднее]
...
/*Переписать SELECT-запрос, разработан-ный в задании 4 так, чтобы в 
расчете сред-него значения оценок использовались оцен-ки только по 
дисциплинам с кодами БД и ОАиП. Использовать WHERE.*/
select FACULTY.FACULTY, GROUPS.PROFESSION, GROUPS.YEAR_FIRST, round(avg(cast(PROGRESS.NOTE as float(4))),2)[среднее]
from FACULTY inner join GROUPS 
	on FACULTY.FACULTY=GROUPS.FACULTY inner join STUDENT 
	on GROUPS.IDGROUP=STUDENT.IDGROUP inner join PROGRESS
...
/*На основе таблиц FACULTY, GROUPS, STUDENT и PROGRESS разработать 
SE-LECT-запрос, в котором выводятся специ-альность, дисциплины и 
средние оценки при сдаче экзаменов на факультете ТОВ. Ис-пользовать
группировку по полям FACULTY, PROFESSION, SUBJECT.*/
select FACULTY.FACULTY, GROUPS.PROFESSION, PROGRESS.SUBJECT, round(avg(cast(PROGRESS.NOTE as float(4))),2)[среднее]
from FACULTY inner join GROUPS 
	on FACULTY.FACULTY=GROUPS.FACULTY inner join STUDENT 
	on GROUPS.IDGROUP=STUDENT.IDGROUP inner join PROGRESS
	on PROGRESS.IDSTUDENT=STUDENT.IDSTUDENT
...
/*На основе таблиц GROUPS, STUDENT и PROGRESS разработать 
SELECT-запрос, в котором определяются результаты сдачи экзаменов.*/
use [univ2];
select FACULTY.FACULTY, GROUPS.PROFESSION, PROGRESS.SUBJECT, round(avg(cast(PROGRESS.NOTE as float(4))),2)[среднее]
from FACULTY inner join GROUPS 
	on FACULTY.FACULTY=GROUPS.FACULTY inner join STUDENT 
...
/*Получить пересечение двух множеств строк, созданных в результате 
выполнения запросов пункта 8+intersect*/
use [univ2];
select FACULTY.FACULTY, GROUPS.PROFESSION, PROGRESS.SUBJECT, round(avg(cast(PROGRESS.NOTE as float(4))),2)[среднее]
from FACULTY inner join GROUPS 
	on FACULTY.FACULTY=GROUPS.FACULTY inner join STUDENT 
...
/*На основе таблицы PROGRESS определить для каждой дисциплины 
количество сту-дентов, получивших оценки 8 и 9. */
